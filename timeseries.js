d3.csv("multiTimeline.csv", function(timeseries) {
    ...
    */
    ...
    dataset.forEach(function(d) {
        d.semana = d3.time.format("%Y-%m-%d").parse(d.semana);
    });

    // sort dataset by month
    dataset.sort(function(x, y){
    return d3.ascending(x.semana, y.semana);
    });
    ...
    */
    ...
    var dataXrange = d3.extent(dataset, function(d) { return d.semana; });
    var dataYrange = [0, d3.max(dataset, function(d) { return d.mobile; })];
    ...
    */
    ...
    var lineI = d3.svg.line()
        ...
        .y(function(d) { return y(d.interface); });

    var line_contextI = d3.svg.line()
        .x(function(d) { return x2(d.semana); })
        .y(function(d) { return y2(d.interface); });


    var lineUx = d3.svg.line()
        .x(function(d) { return x(d.semana); })
        .y(function(d) { return y(d.ux); });

    var line_contextUx = d3.svg.line()
        .x(function(d) { return x2(d.semana); })
        .y(function(d) { return y2(d.ux); });

    var lineM = d3.svg.line()
        .x(function(d) { return x(d.semana); })
        .y(function(d) { return y(d.mobile); });

    var line_contextM = d3.svg.line()
        .x(function(d) { return x2(d.semana); })
        .y(function(d) { return y2(d.mobile); });
    ...
    */
    ...
    context.append("path")
        .datum(dataset)
        .attr("class", "line")
        .attr("d", line_contextI);

    context.append("path")
        .datum(dataset)
        .attr("class", "line2")
        .attr("d", line_contextUx);

    context.append("path")
        .datum(dataset)
        .attr("class", "line3")
        .attr("d", line_contextM);
    ...
    */
    ...
    function brushed() {
        ...
        focus.select(".line").attr("d", lineI);
        focus.select(".line2").attr("d", lineUx);
        focus.select(".line3").attr("d", lineM);
    ...
    // this function dynamically changes the y-axis to fit the data in focus
        ...
        var bisectDate = d3.bisector(function(d) { return d.semana; }).right;
        ...
        if (dataset[iL] !== undefined && dataset[iL-1] !== undefined) {
            ...
            var left_dateBefore = dataset[iL-1].semana,
                left_dateAfter = dataset[iL].semana;
        ...
        if (dataset[iR] !== undefined && dataset[iR-1] !== undefined) {
            ...
            var right_dateBefore = dataset[iR-1].semana,
                right_dateAfter = dataset[iR].semana;
        ...
        var dataSubset = dataset.filter(function(d){ return d.semana >= xleft && d.semana <= xright; });
        var countSubset = [];
        ...
        if(ymax_new == 0 || ymax_new == undefined){
